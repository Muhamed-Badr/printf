		...
		int *debug_num)
{
	...
	/*+D-*/printf("\t\t[+%d] in handle_str()\n", (*debug_num)++);

	if (str != NULL && str_len != 0)
	{
		/*
		 * Write the string to standard output stream (stdout)
		 *  if it bigger than `BUF_SIZE`(1024)
		 */
		if (str_len > BUF_SIZE)
		{
			/*
		...
		else
		{
			...
			_strncpy((buf + (*buf_index + 1)), str, str_len, debug_num);
			*buf_index += str_len;

			/*+D-*/printf("\t\t\t[-%d] out _strncpy()\n", --(*debug_num));
		}
	}
